...
def write_image_data_to_file(logger, config, image_files, data_storage, truth_storage, image_shape, n_channels, affine_storage, truth_dtype=np.uint8):
    '''
    ...
    for idx, set_of_files in enumerate(image_files):
        ...
        count += 1
        print(f"\nReading session {count}/{len(image_files)}    *****************************************************************************************")

        images = reslice_image_set(config, set_of_files, image_shape, label_indices=len(set_of_files) - 1)
...
def write_data_to_file(logger, config,
                       training_data_files,
                       out_file,
                       subject_ids,
    ...
    subject_ids_mp = write_image_data_to_file(logger, config, training_data_files, data_storage, truth_storage, image_shape, truth_dtype=truth_dtype, n_channels=n_channels,
                            affine_storage=affine_storage)

    
    if subject_ids: hdf5_file.create_array(hdf5_file.root, 'subject_ids', obj=subject_ids)
